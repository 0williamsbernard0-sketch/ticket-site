...
export default function CheckoutPage() {
  ...
  const handlePay = () => {
    ...
    url.searchParams.set(
      "successUrl",
      `${window.location.origin}/success/${eventId}?seat=${encodeURIComponent(seatId)}`
    );
    url.searchParams.set(
      "cancelUrl",
      `${window.location.origin}/checkout/${eventId}?seat=${encodeURIComponent(seatId)}&price=${encodeURIComponent(
        String(price)
      )}&section=${encodeURIComponent(section)}&row=${encodeURIComponent(row)}&type=${encodeURIComponent(type)}`
    );
      ...
      <div style={{ padding: 16, maxWidth: 560, margin: "0 auto" }}>
        ...
        >
          ...
          <p style={{ margin: "12px 0 0", fontSize: 12, color: "#6b7280", lineHeight: 1.5, fontWeight: 650 }}>
            You will be redirected to <span style={{ color: "#111827", fontWeight: 900 }}>nowpayment.io</span> to complete payment.
          </p>
        </div>
      </div>
      {/* Sticky pay bar */}
